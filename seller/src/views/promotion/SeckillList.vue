...
async function fetchProducts() {
  const res = await getProductList({ page: 1, page_size: 200, status: 'on_sale' })
...
<template>
  <el-card shadow="never">
    ...
    <el-table v-loading="loading" :data="tableData" stripe>
      ...
      <el-table-column prop="start_time" :label="t('seckill.startTime')" width="170" />
      <el-table-column prop="end_time" :label="t('seckill.endTime')" width="170" />
      ...
      <el-table-column :label="t('common.actions')" min-width="280" fixed="right">
        <template #default="{ row }">
          <el-button v-if="isEditable(row)" link type="primary" @click="handleEdit(row)">
            {{ t('common.edit') }}
          </el-button>
          <el-button v-if="row.status === 'pending'" link type="warning" @click="handleSubmitReview(row)">
            {{ t('seckill.submit') }}
          </el-button>
          <el-button
            v-if="['pending', 'reviewing', 'running'].includes(row.status)"
            link
            type="danger"
          ...
          <el-button v-if="['pending', 'cancelled'].includes(row.status)" link type="danger" @click="handleDelete(row)">
            {{ t('common.delete') }}
          </el-button>
        </template>
      </el-table-column>
