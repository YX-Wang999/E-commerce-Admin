...
import { getCategoryList, getProductList } from '@/api/products'
import {
  createCoupon,
  deleteCoupon,
  disableCoupon,
...
async function fetchOptions() {
  const [prodRes, catRes] = await Promise.all([
    getProductList({ page: 1, page_size: 200, status: 'on_sale' }),
    getCategoryList(),
  ])
  ...
  <el-card shadow="never">
    <template #header>
      <div class="card-header">
        ...
        <el-button type="primary" :icon="Plus" @click="handleCreate">{{ t('coupon.create') }}</el-button>
      </div>
    </template>
    ...
    <el-table v-loading="loading" :data="tableData" stripe>
      ...
      <el-table-column :label="t('common.actions')" min-width="280" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 'published'" link type="warning" @click="handleDisable(row)">
            {{ t('coupon.disable') }}
          </el-button>
