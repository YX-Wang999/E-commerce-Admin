"""Migrate cart from User to Customer."""
...
def migrate_cart_user_to_customer(apps, schema_editor):
    Cart = apps.get_model('cart', 'Cart')
    User = apps.get_model('accounts', 'User')
    Customer = apps.get_model('customers', 'Customer')
    for cart in Cart.objects.all():
        user = User.objects.filter(pk=cart.user_id).first()
        if not user:
            continue
        cart.customer = get_or_create_customer_for_user(User, Customer, user)
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(migrate_cart_user_to_customer, migrations.RunPython.noop),
